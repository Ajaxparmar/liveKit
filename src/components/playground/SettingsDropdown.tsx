...
// Original settings array with camera and video
...
export const SettingsDropdown = () => {
  ...
  // Updated isEnabled function without camera and video
  const isEnabled = (setting: SettingValue) => {
    ...
    if(setting.type === "inputs") {
      const key = setting.key as "mic"; // Removed camera
      return config.settings.inputs[key];
    } else if(setting.type === "outputs") {
      const key = setting.key as "audio"; // Removed video
  ...
  // Updated toggleSetting function
  const toggleSetting = (setting: SettingValue) => {
    ...
    if(setting.type === "chat") {
      ...
      newSettings.inputs[setting.key as "mic"] = newValue; // Removed camera
    } else if(setting.type === "outputs") {
      newSettings.outputs[setting.key as "audio"] = newValue; // Removed video
    }
